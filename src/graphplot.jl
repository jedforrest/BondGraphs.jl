...
function nodelabel(node, showtype)
...
function bgplot(bg::BondGraph; layout=Stress(), interactions=true, ongrid=false, showtype=false)
    allnodes = nodes(bg)
    nodesize = 50 .* [n isa Junction ? 0.75 : 1. for n in allnodes]

    fig, ax, p = GraphMakie.graphplot(bg;
        layout,
        arrow_size = 12,
        arrow_shift = :end,
        node_size = nodesize,
