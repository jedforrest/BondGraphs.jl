...
function BondGraph(rs::ReactionSystem; chemostats=[])
    ...
    for (i, reaction) in enumerate(all_reactions)
        if i > 1 && _is_reverse_of_previous(reaction, all_reactions[i-1])
...
_is_reverse_of_previous(r1, r2) = Set(r1.substrates) == Set(r2.products) && Set(r2.substrates) == Set(r1.products)
